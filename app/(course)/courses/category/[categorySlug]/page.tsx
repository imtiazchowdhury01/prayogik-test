...
//           {/* Server Component for Initial Courses */}
...
// CategorySlugPage.tsx - Optimized Version
import React, { Suspense } from "react";
import type { Metadata, ResolvingMetadata } from "next";
...
export async function generateStaticParams() {
  try {
    ...
    // Batch check for courses count - more efficient than individual calls
    const categoriesWithCourses = await Promise.all(
      categories.map(async (category) => {
        const coursesCount = await getCategoryCoursesCountDBCall(category.slug);
        return { ...category, hasContent: coursesCount > 0 };
      })
    );

    const params = categoriesWithCourses
...
const CategorySlugPage = async ({
  ...
  try {
    // Fetch all required data in parallel instead of sequential calls
    const [categories, categoryName] = await Promise.all([
      getCategoriesDBCall(),
      isFilterType ? Promise.resolve(null) : getCategoryNameBySlugDBCall(slug)
    ]);

    return (
      <section className="min-h-[50vh]">
        {/* Breadcrumbs - Static, no DB calls */}
        <CoursesBreadcrumb isCategoryPage={true} />

        <div className="flex py-6 sm:py-[60px] lg:space-x-5 app-container gap-x-[6px]">
          {/* Sidebar - Pass categories to avoid another DB call */}
          <div className="hidden lg:block lg:w-1/4">
            <Suspense fallback={<CategorySidebarSkeleton />}>
              <CategorySidebar categories={categories} />
            </Suspense>
          ...
          <div className="w-full lg:w-3/4">
            <div>
              {/* Header - Pass categories to avoid client-side DB calls */}
              <CategoryHeader
                categorySlug={slug}
                pageType={isFilterType ? "filter" : "category"}
                categoryName={categoryName!}
                categories={categories} // Pass categories to avoid client-side fetch
              />
            </div>
    ...
    console.error("Error in CategorySlugPage:", error);
      ...
      <section className="min-h-[50vh]">
        ...
        <div className="flex justify-center items-center h-96">
          <div className="text-center">
            <h2 className="text-xl font-semibold text-gray-600">কিছু সমস্যা হয়েছে</h2>
