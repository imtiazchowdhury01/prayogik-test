  ...
  const onDragEnd = (result: DropResult) => {
    ...
    const bulkUpdateData = updatedlessons.map((lesson, index) => ({
      ...
      // position: items.findIndex((item) => item.id === lesson.id),
        position: startIndex + index, // This ensures sequential positions
    }));
            ...
            {lessons.map((lesson, index) => (
              <Draggable key={lesson.id} draggableId={lesson.id} index={index}>
                {(provided) => (
                  <div
                    className={cn(
                      " bg-slate-200 border-slate-200 border text-slate-700 rounded-md mb-4 text-sm",
                      lesson.isPublished &&
                  ...
                  >
                    ...
                    >
                      ...
                      <Pencil
                        ...
                        className="w-4 h-4 mr-3 cursor-pointer hover:opacity-75 transition"
                      />
                    </div>
                  </div>
                )}
