  ...
  prices: z.array(
    z
      ...
      .refine(
        ...
        {
          message:
            "Regular amount must be greater than 0 for paid certifications",
          path: ["regularAmount"],
        }
      )
      .refine(
...
export const CertificationMultiplePriceForm = ({
  initialData,
  certificationId,
}) => {
  ...
  // API functions
  const updateCertificationPrices = async (updatedValues) => {
    try {
      const res = await axios.post(`/api/certifications/prices`, updatedValues);
      return res;
    } catch (error) {
  ...
  const deleteAllPricesFromDb = async (certificationId) => {
    try {
      const response = await axios.delete(
        `/api/certifications/prices?certificationId=${certificationId}`
      );
      return response;
    } catch (error) {
  ...
  const fetchExistingPrices = async (certificationId) => {
    try {
  ...
  const onSubmit = async (values) => {
    try {
      const existingPrices = await fetchExistingPrices(certificationId);

      // Delete existing prices
      if (existingPrices.length > 0) {
        await deleteAllPricesFromDb(certificationId);
      }

      if (values.isFree) {
        // Handle free certification
        await axios.patch(`/api/certifications/${certificationId}`, {
          isUnderSubscription: false,
        });

        const freeCertificationPrices = [
          {
            isFree: true,
            regularAmount: 0,
        ...
        await updateCertificationPrices(freeCertificationPrices);
      } else {
        // Handle paid certification
        const paidCertificationPrices = values.prices
        ...
        await updateCertificationPrices(paidCertificationPrices);
      }

      toast.success("Certification prices updated successfully");
      toggleEdit();
      ...
      console.error("Error updating certification prices:", error);
      toast.error("Failed to update certification prices. Please try again.");
    }
  };
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start p-4 space-x-3 space-y-0 border rounded-md">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormDescription>
                      Check this box if you want to make this certification free
                    </FormDescription>
                  </div>
                </FormItem>
              )}
