...
export function DataTableRowActions({ row, teachers }) {
  ...
  const handleDelete = async (courseRoadmap) => {
    try {
      setLoading(true);
      const response = await clientApi.deleteCourseRoadmap({
        params: {
          id: courseRoadmap?.id,
        },
      });
      toast.success("Course roadmap deleted successfully");
      await revalidatePage("/course-roadmap");
      router.refresh();
      setDialogVisible(false);
    } catch (error) {
      toast.error("Error deleting course roadmap.");
    } finally {
      setLoading(false);
    }
  };
