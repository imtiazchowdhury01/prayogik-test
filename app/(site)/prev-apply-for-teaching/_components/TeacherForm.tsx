...
import { PurchaseType, TeacherExpertiseLevel } from "@prisma/client";
...
export default function RegistrationForm({ planId }: { planId: string }) {
  ...
  const form = useForm({
    ...
    defaultValues: {
      ...
      country: "",
      zipCode: "",
      education: [{ degree: "", major: "", passingYear: "" }],
      certifications: [],
      yearsOfExperience: yearOfExperienceValues[0],
    },
  });
  ...
  useEffect(() => {
    if (data?.user?.id && !isUserLoaded) {
      getUser(data.user.id);
    }
  }, [data?.user?.id, getUser, isUserLoaded]);
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        ...
        if (isMounted) {
          setIsCategories(categoriesData);
        }
      } catch (error) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const teacherRes = await fetch("/api/teacher/apply-for-teacher", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(formattedData),
      });

      const responseData = await teacherRes.json();
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>
                            ...
                            {fieldName === "facebook" && "ফেসবুক"}
                            {fieldName === "youtube" && "ইউটিউব"}
                            {fieldName === "twitter" && "টুইটার"}
                            {fieldName === "others" && "অন্যান্য"}
                          </FormLabel>
                          <FormControl>
                            <Input
                              {...field}
