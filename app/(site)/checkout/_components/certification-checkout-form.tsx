...
const CertificationCheckoutForm = ({
  certification,
  savedPriceType,
  hasDiscount,
  discountedAmount,
  regularAmount,
  isFreeCertification = false,
  isSignedIn,
  ...
  const pricingOptions = useMemo(() => {
    const secondOptionType = certification?.isUnderSubscription
      ? PurchaseType.SUBSCRIPTION
      : PurchaseType.OFFER;

    return {
      regular: {
        type: PurchaseType.CERTIFICATION,
      ...
      subscription: {
        ...
        description:
          subscriptionStatus?.isActive && !subscriptionStatus?.isTrial
            ? certification?.isUnderSubscription
  ...
  const calculateAmount = useCallback(() => {
    if (selectedType === PurchaseType.CERTIFICATION) {
    ...
    if (selectedType === pricingOptions.subscription.type) {
      if (subscriptionStatus?.isActive && !subscriptionStatus?.isTrial) {
        if (certification?.isUnderSubscription) {
      ...
      // Calculate subscription + certification price (for trial users or new subscribers)
  ...
  const handleFreeCertificationEnrollment = async (values: any) => {
    ...
    // if (data.success) {
    //   // certification access success
    //   toast.success("আপনি এই কোর্সটি সফলভাবে এনরোল করেছেন");
    //   router.push(
    //     `/courses/${certification?.slug}/${certification?.lessons[0]?.slug}`
    //   );
    // } else {
    //   toast.error(data?.error || "কোর্স অ্যাক্সেস করা যায়নি");
    // }
    toast.error("কোর্স অ্যাক্সেস করা যায়নি");
  };

  const handlePaidCertificationEnrollment = async (values: any) => {
    const formData = new FormData();
    formData.append("certificationId", certification?.id || "");
    formData.append("type", values.type);
    formData.append("amount", selectedAmount.toString());
    formData.append("isFreeCertification", isFreeCertification);
    ...
    if (result.success) {
      ...
      if (result?.data?.url) {
        ...
        if (session?.user?.email) {
          router.push(`/courses/${certification?.slug}`);
        } else {
          router.push(`/signin`);
        }
      }
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      const courseType = isFreeCertification
        ? "FREE_CERTIFICATION"
        : "PAID_CERTIFICATION";

      switch (courseType) {
        case "FREE_CERTIFICATION":
          await handleFreeCertificationEnrollment(values);
          break;

        case "PAID_CERTIFICATION":
          await handlePaidCertificationEnrollment(values);
          break;
  ...
  const renderSubscriptionPlans = () => {
    if (
      (subscriptionStatus?.isActive && !subscriptionStatus.isTrial) ||
      (subscriptionStatus?.isTrial && certification?.isUnderSubscription) ||
  ...
  function getButtonLabel() {
    ...
    // if (subscriptionStatus?.isTrial && certification?.isUnderSubscription) {
    //   return "ট্রায়াল প্ল্যানে অন্তর্ভুক্ত আছে";
    // }

    if (isFreeCertification) {
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                            ...
                            {purchaseStatus.accessType ===
                              ...
                              <div className="mt-2">
                                {!session?.user?.email ? (
                                  ...
                                  <Link
                                    href={`/courses/${certification?.slug}`}
                                    className="font-semibold text-brand underline hover:text-brand"
                                  >
            ...
            {subscriptionStatus?.isActive &&
            !subscriptionStatus?.isTrial &&
            certification?.isUnderSubscription ? null : (
              <div
                className={` ${
                  isEmailEmtpy && "hidden"
...
export default CertificationCheckoutForm;
