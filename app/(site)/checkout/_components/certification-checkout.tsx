...
import CourseCheckoutForm from "./certification-checkout-form";
...
import { getCertificationBySlug } from "@/services/user";
import CertificationCheckoutForm from "./certification-checkout-form";

const CertificationCheckout = async ({
  cartData,
  errorMessage,
  isPaymentSuccessful,
  transactionId,
  ...
  const { certificationSlug, checkoutType } = cartData?.items[0];
  const certification = await getCertificationBySlug(certificationSlug);
  const allSubscriptionPlans = await getSubscriptionDBCall();
  const cookieStore = cookies();
  const savedPriceType = checkoutType || PurchaseType.CERTIFICATION;
  ...
  if (!cartData || !certification) {
    return <div>Certification not found</div>;
  }

  // Get the first price for regular pricing
  const isFreeCertification = certification?.prices[0]?.isFree || false;
  const regularPrice = certification.prices?.[0];
    ...
    <div className="bg-[#F3F9F9] flex justify-center items-center p-6 xl:p-14 border-b">
      <Card className="app-container mx-auto bg-white p-6 md:p-10 border-0">
        ...
        <div className="flex lg:flex-row flex-col justify-between gap-10 pt-2">
          {/* Left Side - Certification Details */}
          <div className="lg:w-[45%] w-full">
            <Card className="relative p-6">
              ...
              <CardContent className="text-base text-gray-700 space-y-1 px-0">
                <div>
                  ইন্সট্রাক্টর {certification.teacherProfile?.user?.name}
                </div>
                ...
                <div className="flex items-center gap-4 my-2">
                  <div className="flex items-center space-x-[6px]">
                    ...
                    <p className="text-base text-fontcolor-description">
                      {convertNumberToBangla(certification?.courseIds?.length)}{" "}
                      টি কোর্স
                    </p>
                  </div>
                  <div className="flex items-center gap-1">
                    ...
                    {convertNumberToBangla(
                      certification.enrolledStudents?.length || 0
                    )}{" "}
                    শিক্ষার্থী
                  </div>
                </div>
                  ...
                  {hasDiscount && (
                    <div className="space-x-1">
                      ...
                      <span className="text-xs">
                        (স্ট্যান্ডার্ড কোর্সের জন্য প্রযোজ্য)
                      </span>
                    </div>
                  )}
          ...
          <div className="lg:w-[55%] w-full">
            <CertificationCheckoutForm
              ...
              isFreeCertification={isFreeCertification}
