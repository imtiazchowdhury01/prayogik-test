    ...
    <main className="w-full min-h-[70vh]">
      ...
      <section className="w-full relative bg-[url('/gradient-hero-bg.svg')] px-3 bg-cover flex flex-col items-center justify-center object-cover bg-center bg-no-repeat h-96 sm:px-0">
        <div className="flex flex-col items-center justify-between lg:flex-row app-container">
          ...
          <div className="bg-white border-t-[4px] border-secondary-brand p-6 rounded-lg lg:relative absolute z-10 w-full max-w-[330px] left-1/2 sm:left-auto transform -translate-x-1/2  sm:transform-none -bottom-44 lg:bottom-auto sm:right-6">
              ...
              {schedule.map(({ time, description, last }, index) => (
                <li
                  ...
                  className={`relative flex-1 ${
                    !last
                      ? "after:content-[''] after:w-0.5 after:h-[80px] after:bg-primary-brand after:inline-block after:absolute after:-bottom-11 after:left-1.5"
                      : ""
                  }`}
                >
                  <div className="flex items-start w-full">
                    <div className="flex items-center justify-center w-4 h-4 mr-3 text-sm text-white border-2 border-transparent rounded-full bg-primary-brand">
            ...
            {faqData.map((data, ind) => {
              return (
                <AccordionItem
                  value={data.id}
                  key={ind}
                  className="data-[state=open]:border-[1px]  data-[state=open]:shadow-md p-3 rounded-lg data-[state=open]:border-greyscale-200 border-b-[1px] border-greyscale-200 data-[state=open]:bg-white"
                >
                  <AccordionTrigger className="text-lg font-medium data-[state=open]:border-b-[1px] data-[state=open]:border-green-100 hover:no-underline text-fontcolor-title ">
                    {data.question}
                  </AccordionTrigger>
      ...
      <section className="flex flex-col items-center justify-center px-3 py-20 bg-white">
        <h4 className="mb-4 text-3xl font-bold text-center sm:text-4xl text-fontcolor-title">
          এখনই <span className="text-secondary-brand">ওয়ার্কসপ</span>{" "}
          রেজিস্ট্রেশন করুন
        </h4>
        <p className="text-center text-fontcolor-paragraph">
        ...
        <button className="py-4 mt-10 text-base font-semibold text-white transition-all duration-300 rounded-lg hover:opacity-70 bg-primary-brand px-7">
          <Link href={"/workshop/registration"}>রেজিস্ট্রেশন করুন</Link>
        </button>
      </section>
    </main>
  );
