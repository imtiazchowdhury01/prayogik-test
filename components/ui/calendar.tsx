...
export function Calendar({
  ...
  const handleCalendarChange = (_value, _e) => {
    const _event = { target: { value: String(_value) } };
    _e(_event);
  };
  ...
  return (
    <DayPicker
      ...
      components={{
        IconLeft: () => <IconLeft style={{ width: 10 }} className="h-4 w-4" />,
        IconRight: () => (
          <IconRight className="h-4 w-4" style={{ width: 10 }} />
        ),
        Dropdown: ({ onChange, value, caption, children }) => (
          <Select
            value={value}
            onValueChange={(val) =>
              onChange && handleCalendarChange(val, onChange)
            }
          >
            <SelectTrigger
              className={cn(
                buttonVariants({ variant: "ghost" }),
                "pl-2 pr-1 py-2 h-7 w-fit font-medium"
              )}
            >
              <SelectValue placeholder={caption}>{caption}</SelectValue>
            </SelectTrigger>
            <SelectContent className="max-h-[var(--radix-popper-available-height)] overflow-y-auto min-w-[var(--radix-popper-anchor-width)]">
              {children &&
